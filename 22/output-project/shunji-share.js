...
(function (global) {
  ...
  /* ── 讀取後臺配色覆蓋（若有 window.__SHUNJI_COLORS__ 則套用） ── */
  var _AC = (typeof window !== 'undefined' && window.__SHUNJI_COLORS__) || {};
  var _AI = (typeof window !== 'undefined' && window.__SHUNJI_IMAGES__) || {};

  var C = {
    bg:      _AC.bg      || '#0B0907',
    cardTop: _AC.cardTop || '#1B1510',
    cardBot: _AC.cardBot || '#110D0A',
    gold:    _AC.gold    || '#E7CB92',
    goldSoft:_AC.goldSoft|| '#C9A96B',
    ink:     _AC.ink     || '#F0E8DB',
    muted:   _AC.muted   || '#948A7E',
  ...
  function paint(ctx, o, W, mode, H) {
    ...
    /* 後臺圖片快取（同步繪製，需要提前 decode） */
    var _imgs = (typeof window !== 'undefined') ? (window.__SHUNJI_IMG_CACHE__ || {}) : {};

    /* 橫幅圖片（頂部） */
    if (draw && _imgs.banner) {
      try {
        var bW = CW - P * 0.6, bH = bW * 9 / 32;
        var bX = CXX + (CW - bW) / 2;
        ctx.save();
        ctx.beginPath();
        ctx.roundRect(bX, y - 60, bW, bH, 10);
        ctx.clip();
        ctx.drawImage(_imgs.banner, bX, y - 60, bW, bH);
        ctx.restore();
        y += bH - 20;
      } catch(e){}
    }
    ...
    if (draw) {
      if (_imgs.logo) {
        try {
          var lSize = 52;
          var lX = W/2 - (trackedWidth(ctx, CFG.brand + '　' + CFG.brandSub, 4) / 2) - lSize - 10;
          ctx.save();
          ctx.beginPath();
          ctx.roundRect(lX, y - lSize + 10, lSize, lSize, 8);
          ctx.clip();
          ctx.drawImage(_imgs.logo, lX, y - lSize + 10, lSize, lSize);
          ctx.restore();
        } catch(e){}
      }
    ...
    var totalH = px(y + 44 + DEPTH);

    /* 浮水印（繪完主體後疊在中央） */
    if (draw && _imgs.watermark && H) {
      try {
        ctx.save();
        ctx.globalAlpha = 0.08;
        var wmW = CW * 0.55, wmH = wmW;
        ctx.drawImage(_imgs.watermark, CXX + (CW - wmW)/2, H/2 - wmH/2, wmW, wmH);
        ctx.restore();
      } catch(e){}
    }

    /* 印章（右下角） */
    if (draw && _imgs.stamp && H) {
      try {
        ctx.save();
        ctx.globalAlpha = 0.85;
        var stW = 140, stH = 140;
        ctx.drawImage(_imgs.stamp, CXX + CW - stW - P/2, H - stH - M - DEPTH - 10, stW, stH);
        ctx.restore();
      } catch(e){}
    }
  ...
  /* ── 預載後臺圖片（base64 → HTMLImageElement） ── */
  function preloadAdminImages() {
    var imgs = (typeof window !== 'undefined') ? (window.__SHUNJI_IMAGES__ || {}) : {};
    var keys = ['logo', 'banner', 'watermark', 'stamp'];
    var cache = window.__SHUNJI_IMG_CACHE__ = window.__SHUNJI_IMG_CACHE__ || {};
    var promises = keys.map(function(k) {
      if (!imgs[k]) { cache[k] = null; return Promise.resolve(); }
      if (cache[k]) return Promise.resolve();
      return new Promise(function(res) {
        var img = new Image();
        img.onload = function(){ cache[k] = img; res(); };
        img.onerror = function(){ cache[k] = null; res(); };
        img.src = imgs[k];
      });
    });
  ...
  function build() {
    ...
    state.building = preloadAdminImages()
